...
async def purge(m):
    ...
    if rank != "member":
        ...
        if necessary_evil.can_delete_messages or necessary_evil.can_delete_messages is None:
            if m.reply_to_message is None:
                await bot.reply_to(m, "Reply to a message to purge.")
                return
        ...
        await bot.send_message(m.chat.id, "Admin only.")
